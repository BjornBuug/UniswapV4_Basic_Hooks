    ...
    Permit2Forwarder
{
    ...
    mapping(uint256 tokenId => PositionConfigId configId)
        internal positionConfigs;

    /// @notice an internal getter for PositionConfigId to be used by Notifier
    function _positionConfigs(
        uint256 tokenId
    ) internal view override returns (PositionConfigId storage) {
    ...
    modifier onlyValidConfig(uint256 tokenId, PositionConfig calldata config)
        override {
        if (positionConfigs[tokenId].getConfigId() != config.toId())
            revert IncorrectPositionConfigForTokenId(tokenId);
        _;
    }
    ...
    function _handleAction(
        ...
        if (action < Actions.SETTLE) {
            ...
            if (action == Actions.SETTLE_PAIR) {
                ...
                (Currency currency0, Currency currency1, address to) = params
